...
const fetchData = async () => {
  try {
    ...
    const [userRes, contactRes] = await Promise.all([
      axios.get('https://www.gael-lobato.space/api/auth/users', config),
      axios.get('https://www.gael-lobato.space/api/contact', config)
    ])
...
const registerUser = async () => {
  ...
  try {
    await axios.post('https://www.gael-lobato.space/api/auth/register', newUser.value)
    await fetchData()
    closeModal()
  } catch (err) {
